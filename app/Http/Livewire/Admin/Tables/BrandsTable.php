...
class BrandsTable extends DataTableComponent
{
    ...
    public function deleteSelected(): void
    {
        if (count($this->getSelected()) > 0) {
            ...
            Notification::make()
                ...
                ->body(__('components.tables.messages.delete', ['name' => strtolower(__('layout.forms.label.brand'))]))
                ->success()
                ->send();
        }
    ...
    public function enabled(): void
    {
        if (count($this->getSelected()) > 0) {
            ...
            Notification::make()
                ...
                ->body(__('components.tables.messages.enabled', ['name' => strtolower(__('layout.forms.label.brand'))]))
                ->success()
                ->send();
        }
    ...
    public function disabled(): void
    {
        if (count($this->getSelected()) > 0) {
            ...
            Notification::make()
                ->title(__('components.tables.status.updated'))
                ->body(__('components.tables.messages.disabled', ['name' => strtolower(__('layout.forms.label.brand'))]))
                ->success()
                ->send();
        }
    ...
    public function columns(): array
    {
        return [
            Views\Column::make(__('layout.forms.label.name'), 'name')
                ->sortable()
                ->searchable()
                ->view('admin.livewire.tables.cells.brands.name'),
            Views\Column::make(__('layout.forms.label.website'), 'website')
                ->view('admin.livewire.tables.cells.brands.site'),

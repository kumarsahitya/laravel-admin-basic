...
use App\Notifications\AdminSendCredentials;
...
class CreateAdminUser extends AbstractBaseComponent
{
    ...
    public function updatedRoleId(int $id): void
    {
        ...
        $this->is_admin = $chooseRole->name === config('system.users.admin_role');
    }
    ...
    {
        return view('admin.livewire.settings.management.create', [
            'roles' => Role::query()
                ->select(['id', 'display_name', 'name'])
                ->where('name', '<>', config('system.users.default_role'))
                ->get(),
        ]);
    }
}

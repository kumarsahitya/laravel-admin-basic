                ...
                >
                    <div class="rounded-md bg-white shadow-xs dark:bg-secondary-800">
                        <div class="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                            <button wire:click="$emit('openModal', 'modals.delete-customer', {{ json_encode([$customer->id]) }})" type="button" class="block w-full px-4 py-2 text-sm leading-5 text-left text-secondary-700 hover:bg-secondary-100 hover:text-secondary-900 focus:outline-none dark:text-secondary-300 dark:hover:text-white dark:hover:bg-secondary-700" role="menuitem">
    ...
    <div class="py-6">
        <div class="sticky top-0 -mx-6 px-6 bg-secondary-100 dark:bg-secondary-900">
            <div class="sm:border-b sm:border-secondary-200 dark:border-secondary-700">
                <div class="sm:hidden">
                    <x-admin::forms.select x-model="currentTab" aria-label="{{ __('words.selected_tab') }}" class="pr-10">
        ...
        <div class="mt-6">
            <div x-show="currentTab === 'profile'">
                <livewire:customers.profile :customer="$customer" />
            </div>
            <div x-cloak x-show="currentTab === 'address'">
                <livewire:customers.addresses :adresses="$customer->addresses" />
            </div>
            <div x-cloak x-show="currentTab === 'orders'">
                <livewire:customers.orders :customer="$customer" />
            </div>
        </div>
    </div>

@if ($paginator->hasPages())
    ...
    @else
        <x-admin::buttons.secondary type="button" class="relative" wire:click="previousPage" rel="prev" aria-label="{{ __('Previous') }}">
            {{ __('Previous') }}
    ...
    @if ($paginator->hasMorePages())
        <x-admin::buttons.secondary type="button" wire:click="nextPage" rel="next" class="ml-3 relative" aria-label="{{ __('Next') }}">
            {{ __('Next') }}
        </x-admin::buttons.secondary>
    @else

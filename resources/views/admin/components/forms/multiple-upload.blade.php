...
<div x-data="uploadsMultiple" class="relative">
            ...
            <div class="space-y-1 text-center" wire:loading.remove wire:target="files">
                ...
                <p class="mt-1 text-sm text-secondary-500 dark:text-secondary-400">
                    <span class="font-medium text-primary-600 group-hover:text-primary-500 dark:group-hover:text-primary-500 focus:outline-none focus:underline transition duration-150 ease-in-out">
                        {{ __('words.components.files.upload_file') }}
                    </span>
                    {{ __('components.files.drag_n_drop') }}
                </p>
                <p class="mt-1 text-xs text-secondary-500 dark:text-secondary-400">
                    {{ __('components.files.file_type_size', ['size' => 1]) }}
                </p>
            </div>
            <input
                accept="image/*"
            ...
            @foreach($images as $image)
                <div class="relative flex flex-col items-center overflow-hidden h-24 text-center bg-secondary-100 border rounded-md select-none dark:bg-secondary-700 dark:border-secondary-700">
                    ...
                    <div wire:loading.class.remove="hidden" wire:target="removeMedia({{ $image->id }})" class="absolute h-full w-full flex items-center justify-center text-center opacity-100 focus-within:opacity-100 inset-0 z-50 bg-secondary-800 bg-opacity-75">
                        <x-admin::loader wire:loading wire:target="removeMedia({{ $image->id }})" class="text-white" />
                    </div>
                </div>
            @endforeach
        </div>
    ...
    <div class="hidden mt-2 flex items-center" wire:loading.class.remove="hidden" wire:target="files">
        <x-admin::loader wire:loading wire:target="files" class="text-primary-600" />
        <span class="ml-1.5 text-sm text-secondary-500 dark:text-secondary-400">
            {{ __('components.files.uploading') }}
        </span>
    </div>
</div>
